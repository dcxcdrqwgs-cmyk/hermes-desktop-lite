...
### 6. ✅ 任务管理（Tasks）
TODO/IN_PROGRESS/DONE 状态流转、进度统计、增删改。
...
**实现原理**：
- 工作区列表存储在 `~/.hermes/hermes-slate-desk/config.json` 中
...
```bash
# 克隆仓库
git clone https://github.com/your-username/hermes-slate-desk.git
...
- `println!` 日志输出
- 查看 `~/.hermes/hermes-slate-desk/sessions.db`（DB Browser for SQLite）
